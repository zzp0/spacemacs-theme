...
(defun create-spacemacs-theme (variant theme-name)
  ...
  (let ((class '((class color) (min-colors 89))) ;;              ~~ Dark ~~                              ~~ Light ~~
        ...
        (mode-line-bg1 (if (eq variant 'dark) (if (true-color-p) "#403654" "#444444") (if (true-color-p) "#cfcfe6" "#d7d7ff")))
...
;;;;; smart-mode-line
     ...
     `(sml/read-only           ((,class (:foreground ,war :weight bold))))
     `(sml/outside-modified    ((,class (:foreground ,bg1 :background ,war :weight bold))))
